...
Data harmonisation will address phenotype labelling inconsistencies, ancestry stratification, and cohort-specific batch effects. The overall module architecture of the project is depicted in Figure~\ref{fig:architecture}, and the harmonisation workflow is outlined in Algorithm~\ref{alg:harmonisation}.
...
\caption{High-level computational module architecture of the LBD multi-omics project. Data flow proceeds left to right: raw data from five public repositories pass through the harmonisation module, feed into five single-layer analysis modules, are integrated by MOFA+ and SNF, and finally enter the downstream analysis and output stage.}
\label{fig:architecture}
\end{figure}

\subsection{Stage 2: Single-Layer Analysis (Prior to Integration)}

Before integration, each omics layer will be independently analysed to characterise LBD-associated signals and estimate effect sizes (see Figure~\ref{fig:workflow}):
...
\caption{End-to-end analysis workflow for the LBD multi-omics project. The pipeline proceeds top-to-bottom through four stages: (1) data acquisition and harmonisation, (2) parallel single-layer analysis of five omics modalities, (3) multi-omics integration via MOFA+ and SNF converging to an integrated molecular signature, and (4) downstream analysis producing a biomarker model, convergent pathway map, and a prioritised drug target list.}
\label{fig:workflow}
\end{figure}
