...
The Python reimplementation is a proper software package. The `src/lewy/` directory contains **five focused modules**. `data.py` handles loading, label harmonisation, and the LOD filter. `features.py` runs the differential analysis. `model.py` builds and cross-validates the elastic net classifier. `evaluate.py` computes ROC curves and handles validation cohorts. `plots.py` generates all figures. Each module is independently importable — you can call `from lewy.model import build_classifier` in any notebook or script without running the whole pipeline. There are **two CLI entry points**. `pipeline.py` runs the full recreation: differential analysis, classifier training, validation, figures. `protein_auc_pipeline.py` runs the improvement, completely independently — it does not touch or depend on the original pipeline code at all. Both are tested on **Google Colab** with 100-repeat cross-validation, running end-to-end in under an hour on a free CPU runtime.
...
The paper's major limitation is that it examines **individual proteins in isolation only for DDC**. They report DDC's standalone AUC of 0.91 as a headline result, but they stop there. The other 657 proteins are invisible in any individual analysis. The improvement is to run this **for all 664 proteins**. For each protein, fit a logistic regression on three features: the protein's NPX value, patient age, and patient sex. Compute AUROC. This takes roughly 3 minutes on Colab CPU. The result is a **full ranked biomarker landscape** — every protein's individual discriminative power for both comparisons, with panel proteins flagged. From an engineering standpoint this is a clean vectorised loop with a tqdm progress bar and JSON output. It runs through a separate entry point so the original pipeline is never modified. The two scripts are completely independent and can be run in either order.
